...
// getBackspaceSequence returns the byte sequence that moves the terminal
// cursor from the end of r.buf back to position r.idx.
//
// Expectations:
...
//   - Does not emit sep escape for single-line input (no line wrap)
func (r *RuneBuffer) getBackspaceSequence() []byte {
	// sep[i] is true when rune i-1 is the first rune on a new terminal line.
	// We index by rune position (not visual column) so CJK double-width chars
	// are handled correctly. The original code stored visual-column positions
	// and used them as rune indices — correct only for ASCII.
	var sep = map[int]bool{}
	if r.width > 0 {
		col := r.promptLen()
		for i := 0; i < len(r.buf); i++ {
			w := runes.Width(r.buf[i])
			if col/r.width < (col+w)/r.width {
				// Rune i straddles or ends exactly at a line boundary;
				// after backing past it the cursor crosses to the previous line.
				sep[i+1] = true
			}
			col += w
		}
	...
	for i := len(r.buf); i > r.idx; i-- {
		// Emit Width(rune) backspaces so CJK (2-column) chars are fully backed over.
		w := runes.Width(r.buf[i-1])
		for j := 0; j < w; j++ {
			buf = append(buf, '\b')
		}
		if sep[i] {
			// Up one line, go to the start, then move right to the terminal edge.
