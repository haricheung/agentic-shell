...
- **Cross-task memory** — episodic (successes) + procedural (failures) written to `~/.cache/agsh/memory.json`; calibrated at every plan
- **Structured task logs** — per-task JSONL with full LLM prompts, tool calls, criterion verdicts, corrections, replans (`~/.cache/agsh/tasks/<id>.jsonl`)
...
- **Safety gate** — file-destructive commands (`rm`, `find -delete`, `rm` in loops/xargs) require confirmation; generated files land in `~/agsh_workspace/` not the project root
...
# One-shot mode
go run ./cmd/agsh "find my largest video files in Downloads"
...
| Path | Contents |
|---|---|
| `~/.cache/agsh/memory.json` | Episodic + procedural memory across sessions |
| `~/.cache/agsh/audit.jsonl` | Structured audit events |
| `~/.cache/agsh/tasks/<id>.jsonl` | Per-task log: LLM prompts, tool calls, verdicts, replans |
| `~/.cache/agsh/debug.log` | Internal role debug logs |
| `~/agsh_workspace/` | Files generated by the executor land here |

Watch debug output live:
```bash
tail -f ~/.cache/agsh/debug.log
```
...
```
cmd/agsh/          — entry point: REPL, one-shot, wiring
internal/
