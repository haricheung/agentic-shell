...
The diagram shows the full operational flow. The **user** submits a task to the **Perceiver**, which forwards a structured goal to the **Metaagent**. Inside the Metaagent, the Planner decomposes the goal and fans it out as numbered subtasks (intent · criteria · context) to N parallel **Effector Agents**. Each Effector Agent runs its own internal fast loop — Executor → Validator → Dreamer — and returns phase feedback (partial results or uncertainty signals) upward to the Metaagent. The Metaagent's Meta-Validator and **GGS** evaluate the aggregate outcome, adjust the plan if needed, and eventually deliver the final result back to the user. **Shared Memory** sits above the hierarchy: both the Metaagent and the Effector Agents read from it (fetch memory) and write back to it asynchronously via their Dreamer components (consolidate memory).
