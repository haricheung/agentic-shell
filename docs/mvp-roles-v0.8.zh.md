...
图示展示了完整的操作数据流。**用户**提交任务至**感知器**，感知器将结构化目标转发给**规划器（元智能体）**。元智能体内部，规划调度将目标分解并以编号子任务（意图 · 指标 · 上下文）的形式扇出至 N 个并行**效应器智能体**。每个效应器智能体运行其内部快速回路——执行器 → 验证器 → 梦境模块——并将阶段性反馈（阶段性成果或不确定性信号）向上返回给元智能体。元智能体中的验证器与**目标梯度求解器（GGS）**评估综合结果，必要时调整任务，最终将最终结果反馈给用户。**共享记忆体**位于层级顶端：元智能体和效应器智能体均可读取（获取记忆），并通过各自的梦境模块异步写回（重整记忆）。
