...
artoo 是一个用于在本地计算机上自主执行任务的多智能体系统。用户提交一条自然语言请求；artoo 将其分解为子任务，并行或按序执行，依据机器可验证的标准逐一校验结果，在结果不达标时进行重规划——全程无需人工干预。核心创新在于借鉴经典控制理论的双层嵌套控制回路：快速内回路（Executor + Agent-Validator）实时处理每个子任务的纠正，而慢速外回路（Goal Gradient Solver (**GGS**)）对完整任务结果计算损失函数并发出有数学依据的重规划指令。每次任务的经验被编码为 Megram 并存储在 **MKCT** (Megram / Knowledge / Common Sense / Thinking) 认知记忆金字塔中，Planner 在每次新任务前查询该金字塔，以充分利用过往成功经验并规避已知失败模式。
...
人类组织需要协商，因为信息是分散的——每个成员只看到自己的切片。本系统彻底消除了这种信息不对称。Metaagent 在其操作范围内是全知的：它编写了每一个子任务，持有完整的 TaskSpec，观察快速回路中累积的每一个纠正信号，接收每一个携带完整证据链的 SubTaskOutcome，并访问所有历史记忆。没有任何需要协商的信息。
...
| 时间尺度 | 决策 | 执行 | 纠正 | 判断标准 |
|---|---|---|---|---|
| 快速（动作级） | Agent-Validator 发出重试反馈 | Executor 重新运行 | Agent-Validator 重新测量差距 | 子任务成功标准 |
| 中速（任务级） | Goal Gradient Solver (GGS) 调整规划 | Planner 重新调度 | Meta-Validator 重新合并结果 | 用户意图在可信范围内 |
| 慢速（系统级） | Dreamer 综合新策略 | 下一次任务规划 | 下一次 Meta-Validator 周期 | 跨任务长期质量 |

这不是三个独立的机制，而是在不同时间尺度上运行的同一闭环控制结构。时间尺度的分离确保快速重试不会淹没 Planner，系统级整合不会阻塞执行。

### 3. Goal Gradient Solver (GGS) 作为动态差分控制器
...
正式定义：GGS 是控制器；Meta-Validator 是传感器；Planner 是执行机构；Effector Agent 是被控对象。收敛熔断（连续 2 次恶化 → 强制 `abandon`）执行第二定律：在超过发散点后继续运行是资源消耗，而非尽力而为。

### 4. 验证作为首要控制机制

系统没有独立的重试策略、超时管理器或错误处理器。Agent-Validator 的差距测量**就是**重试标准。差距非零则继续执行；差距无法弥合则上报失败，由 Meta-Validator 决定下一步。这确保重试标准永远不会偏离成功标准——两者是同一信号。

### 5. 具有独立委托人的 Auditor

Auditor 的委托人是人类操作员，而非任何智能体。这是结构性属性，而非访问控制设置。一个可以被 Planner 指挥的 Auditor 是有汇报职能的下属，而非 Auditor。

两个属性必须无条件保持：
- **不干预性**：Auditor 从不向任何智能体发送消息。一旦发出纠正信号，它就成了第二个控制器，回路结构随之崩溃。
- **不可变的隔离日志**：审计日志独立于 Shared Memory，任何智能体不得读取、修改或压制。

操作反馈回路在设计上是自包含的——这对运行效率有利，但会产生关于系统自身行为的结构性盲点。Auditor 是刻意的补偿：提供这些回路无法自我提供的外部视角。

### 6. 集中式 Shared Memory

采用单一持久化存储，而非每个智能体的私有记忆。这一选择直接源于 Effector Agent 的临时性：为每个子任务新建、完成后即销毁的智能体创建私有记忆毫无意义——不存在持续的身份来积累知识。集中式记忆还避免了 O(n²) 的对账问题：若 N 个智能体持有可能分歧的私有存储，对齐它们需要 N² 次比较和仲裁机制。

Effector Agent 从不直接查询 Shared Memory。访问是**间接的**：R2 在规划时读取并校准记忆，然后将相关上下文注入每个 `SubTask.context`。Executor 直接对内存存储进行查询将创建不可观测的信息路径，并复制本属于 R2 的校准逻辑。

### 7. 可观测消息总线

所有角色之间的通信均通过单一共享消息总线传递。任何角色不得直接调用另一个角色。这确保：(a) 每条角色间消息都是可观测事件——Auditor 无需对各角色埋点即可只读接入总线；(b) 角色完全解耦，可独立测试；(c) 总线是非阻塞的——慢速订阅者以记录警告的方式丢弃消息，而不是对发布者施加背压。

这是一等架构约束，而非基础设施细节。必须在实现任何角色之前建立。在点对点调用已就位后再补充可观测性需要重构整个通信层。

### 8. 认知记忆基底

原始任务经验被编码为 Megram——携带量级（f）、效价（σ）和衰减率（k）的原子元组。Megram 以 (space, entity) 标签为键累积存储在 LevelDB 中。Planner 在每次任务前查询两个卷积通道：注意力通道（无符号能量——关注哪里）和决策通道（有符号偏好——该做什么）。当超过显著性阈值时，Dreamer 后台引擎将 Megram 簇提升为永久有效的常识（C 级）标准操作程序（SOP），并通过 Trust Bankruptcy 机制降级过时知识。记忆层从不阻塞操作热路径——所有写入均为即发即忘。
...
- **时间成本** — 用户感受到的延迟。主要驱动因素是关键路径上*顺序* LLM 调用的数量。并行调用增加 token 成本但不增加时间成本。单子任务的最少顺序链为：R1 → R2 → R3 → R4a → R4b = 5 次调用。快速回路每次重试额外增加 2 次（R3 + R4a），每次重规划再增加 2 次（R2 + R4b）。
...
所有角色间通信必须通过共享消息总线传递，Auditor 可以只读观察者身份接入。不允许角色间直接点对点调用——每条消息必须是可路由的。总线是非阻塞的：慢速订阅者会以记录警告的方式丢弃消息，而不是对发布者施加背压。
...
| ID | 角色 | 层级 | 回路位置 | 问责范围 |
|---|---|---|---|---|
| R1 | Perceiver | 入口点 | 参考信号 | 若任务被误解，此角色负责 |
| R2 | Planner | Metaagent | 执行机构 | 若在有效执行的情况下目标仍未达成，此角色负责 |
| R3 | Executor | Effector Agent | 被控对象（快速回路） | 若可行的子任务未被正确执行，此角色负责 |
| R4a | Agent-Validator | Effector Agent | 传感器 + 控制器（快速回路） | 若子任务结果与目标之间的差距未被解决或上报，此角色负责 |
| R4b | Meta-Validator | Metaagent | 传感器（中速回路） | 若合并结果在可信范围外被接受，或任务被静默放弃，此角色负责 |
| R5 | Shared Memory | 基础设施 | 认知基底 | 若有效数据丢失、损坏或被错误检索，此角色负责 |
| R6 | Auditor | 基础设施 | 侧向观察者 | 若系统性失败未被检测并上报给人类操作员，此角色负责 |
| R7 | Goal Gradient Solver (GGS) | Metaagent | 控制器（中速回路） | 若重规划方向错误、过于保守、过于激进，或未能逃脱局部最小值，此角色负责 |
...
- 来自用户的自由格式文本（REPL (Read-Eval-Print Loop)；或单次 CLI (Command-Line Interface)）
...
**回路位置**：快速回路的传感器 + 控制器。与每个 Executor 实例一一配对。
...
**使命**：充当所有 Effector Agent 的汇聚门。R4b 收集每个 SubTaskOutcome，合并输出，并做出二元决策：接受任务结果（所有标准已满足）或向 GGS 发送重规划请求。R4b 是中速回路的传感器——它观察所有子任务结果的全貌，并将聚合差距呈现给控制器（R7）。
...
- 写入 Shared Memory——GGS 是所有路径上 R5 的唯一写入者
...
**使命**：充当系统持久的认知基底。R5 将经验以 Megram 的形式积累，通过 Dreamer 后台引擎将反复出现的模式提升为跨任务 SOP，并衰减过时知识——同时永不阻塞操作热路径。

**回路位置**：基础设施层。由 GGS（R7）写入；由 Planner（R2）读取。

### MKCT Pyramid (Megram · Knowledge · Common Sense · Thinking)
...
| 层级 | 名称 | 衰减 k | 描述 |
|---|---|---|---|
| M | Megram | 依量化矩阵 | 原始情节事实；创建时的默认层级 |
| K | Knowledge | 同 M | 任务范围缓存；由 Dreamer GC (Garbage Collector) 清理 |
| C | Common Sense | 0.0（永久有效） | 提升的 SOP 或约束；由 LLM 从 M 簇中提炼 |
| T | Thinking | 0.0（永久有效） | 系统人格和智能体法则；硬编码在系统提示词中 |
...
标签约定：
- **微事件**（动作状态）：`space="tool:<name>"`，`entity="path:<target>"` — 每个 blocked_target 一个 Megram
- **宏事件**（终止状态）：`space="intent:<intent-slug>"`，`entity="env:local"` — 每个路由决策一个 Megram
...
| `success` (D≤δ) | 0.80 | +1.0 | 0.05 | 最佳实践；Planner 可直接复用 |
...
C/T 级条目 k=0.0（永久有效，直至 Trust Bankruptcy）。
...
### Dreamer — 离线巩固引擎 (Offline Consolidation Engine)

以 5 分钟为间隔在后台 goroutine 中运行。永不阻塞操作热路径。

**MVP（v0.8）——向下流动（已激活）**：
- *物理遗忘*（Λ_gc）：M/K 级条目中实时 `M_attention < 0.1` → 从 LevelDB 中硬删除
- *Trust Bankruptcy*（Λ_demote）：C 级条目中实时 `M_decision < 0.0` → 撤销时间豁免；k 恢复为 0.05；降级至 K 级

**向上流动——巩固（延迟至 v0.9）**：
- 将具有相同 `(space, entity)` 标签对的 Megram 进行聚类
- `M_attention ≥ 5.0 且 M_decision ≥ 3.0` → 调用 LLM 提炼最佳实践 → 新 Megram（Level=C，k=0.0）
- `M_attention ≥ 5.0 且 M_decision ≤ −3.0` → 调用 LLM 提炼约束 → 新 Megram（Level=C，k=0.0）
...
错误更正通过追加负 σ Megram 来实现，而非修改现有记录。
...
| 提供 C 级读取 | Planner（R2） | `[]SOPRecord` |
| 提供 M/K 级读取 | Planner（R2） | `Potentials{Attention, Decision, Action}` |
...
损失在减少，但方法在逻辑上是错误的。这令人生疑——系统可能在幻觉式成功、攻击评估标准，或在错误的吸引域中收敛。正确的应对是不信任改善趋势并更换方法。未来 Dreamer 的向上巩固遍历将把这种模式识别为系统性评估偏差。
...
- **动作状态**（change_path、refine、change_approach、break_symmetry）：为 `blocked_targets` 中的每个条目写入一个 Megram；标签 = `(tool:<name>, path:<target>)`
- **终止状态**（accept、success、abandon）：写入一个带有标签 `(intent:<task-intent-slug>, env:local)` 的 Megram
...
| P 阈值参数化为 ρ | 为 Dreamer 引导调优做准备：ρ 将根据 MKCT 金字塔中的历史失败模式按任务类型可调 |
| R5 Shared Memory 重新设计：MKCT 金字塔 + 双通道卷积 | 关键词扫描 JSON 存储无法支持跨任务 SOP 提升、衰减加权规避，以及方法级与路径级失败的结构化区分 |
...
| TaskSpec 不携带 success_criteria——R2 推导所有标准 | R1 提示词；R2 Planner 提示词 |
...
| GGS 是 R5 Shared Memory 的唯一写入者 | R7 代码；R4b 不再写入 MemoryEntry |
| 动作状态下每个 blocked_target 对应一个 Megram | R7 写入路径 |
| Megram 写入为即发即忘；GGS 从不阻塞在记忆 I/O 上 | R5 异步写入队列 |
| 记忆返回结构化数据；R2 格式化为提示词 | R5 接口契约 |
| C 级 Megram k=0.0，直至 Trust Bankruptcy | R5 Dreamer 引擎 |
...
| 失败模式 | 责任角色 |
|---|---|
| 用户原始意图未被忠实传入 TaskSpec | R1 — Perceiver |
| 模糊意图被误解；task_criteria 错误 | R2 — Planner |
| 尽管子任务执行有效，目标仍未达成 | R2 — Planner |
| 放弃时：仅输出失败消息，未提供任何部分结果 | R2 — Planner |
| 可行的子任务未被正确执行 | R3 — Executor |
| 子任务输出与目标之间的差距未被解决或上报 | R4a — Agent-Validator |
| 失败的子任务被接受为成功；合并结果未通过 task_criteria | R4b — Meta-Validator |
| 重规划方向错误；局部最小值未被逃脱；预算判断有误 | R7 — Goal Gradient Solver (GGS) |
| 有效经验数据丢失、损坏或被错误检索 | R5 — Shared Memory |
| 系统性失败未被检测并上报给操作员 | R6 — Auditor |
...
| 组件 | 所需工作 |
|---|---|
| GGS 超参数调优 | 基于 Auditor 会话数据对 α、β、λ、w₁、w₂、ε、δ、ρ、θ 进行经验性校准 |
...
| Dreamer 向上巩固 | LLM 从 M 级簇中提炼 C 级 SOP/约束；FinalResult 触发的安定延迟 |
...
| Dreamer 模式迁移引擎（Dreamer Schema Transfer Engine） | 对高分 Megram 进行语义因式分解；生成假设 Megram 以发明新颖工具组合 |
| Dreamer 认知失调 Megram | 在软覆写时生成失调 Megram（高 f，负 σ）；在夜间 Dreamer 周期中粉碎过时 SOP 的可信度 |
